...
const ArchiveSection = ({
  ...
  const restoreFromArchive = async (taskId: string) => {
    ...
    try {
      toast.info(currentLanguage === "en" ? "Removing The Task From Archive" : "جاري إلغاء أرشفة المهمة");
      const { data } = await axios.put(`${DOMAIN}/api/tasks/archive/${taskId}`, { archived: false });
      dispatch(tasksActions.updateTaskData(data));
      toast.success(currentLanguage === "en" ? "Task Removed From Archive Succefully" : "تم إلغاء المهمة بنجاح");
    } catch (error) {
      handleRequestError(error, currentLanguage === "en" ? "An error occured during remove task from archive" : "حدث خطأ أثناء إلغاء أرشفة المهمة");
    }
  };
  return (
    currentTab === "archive" && (
